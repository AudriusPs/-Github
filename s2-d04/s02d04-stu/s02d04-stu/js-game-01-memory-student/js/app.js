document.addEventListener('DOMContentLoaded', () => {
    ...
    const rezultDisplay = document.querySelector('#rezult')
    ...
    function checkForMatch() {
        ...
        if (optionOneId == optionTwoId) {
            ...
            cards[optionOneId].setAttribute('src', 'img/leaves.png')
            cards[optionTwoId].setAttribute('src', 'img/leaves.png')
            alert('Neacpejai. Bandyk dar karta.')
        }
        cardsChosen = []
        cardsChosenId = []
        rezultDisplay.textContent = cardsWon.length
        if (cardsWon.length === cardArray.length / 2) {
            rezultDisplay.textContent = 'Sveikname! acpejai visas kortas'
            if (confirm('Zaisi dar karta?')) {
                document.location.href = ''
            } else {
                rezultDisplay.textContent = 'Iki kito carto!'
            }
        }
    }
